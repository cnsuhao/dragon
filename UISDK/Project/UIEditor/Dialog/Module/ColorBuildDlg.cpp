...
void CColorViewDlg::Reload()
{
	...
	IColorRes* pColorRes = m_pSkin->GetColorRes();
	if (NULL == pColorRes)
		return;
	...
	long nCount = pColorRes->GetColorCount();
	for (long i = 0; i < nCount; i++)
	{
		IColorResItem*  pInfo = pColorRes->GetColorResItem(i);
		if (pInfo)
		{
			...
			if (pColorRes)
			{
				pColorRes->GetColor((BSTR)pInfo->GetID(), &pcol);
...
void  CColorViewDlg::OnBtnAdd(UINT uNotifyCode, int nID, CWindow wndCtl)
{
	...
	if( IDOK == dlg.DoModal() )
	{
		...
		if (pColorRes)
		{
			pColorRes->GetColor((BSTR)dlg.m_strId.c_str(), &pcol);
...
void  CColorViewDlg::OnBtnRemove(UINT uNotifyCode, int nID, CWindow wndCtl)
{
	...
	IColorManager* pColorMgr = m_pSkin->GetColorManager();
	if (NULL == pColorMgr)
		return ;
