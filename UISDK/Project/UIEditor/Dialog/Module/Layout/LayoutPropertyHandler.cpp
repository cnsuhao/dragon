...
#pragma region	// 直接显示xml属性
    ...
    {
        IPropertyCtrlGroupItem* pParentNode = m_pPropertyCtrl->InsertGroupItem(_T("xml"), _T("自有属性"), UITVI_ROOT, UITVI_LAST, 0);
...
void LayoutPropertyHandler::AttachObject_IDAttrib(ISkinRes* hSkin, IObject* pObj, const String& strID)
{
    ...
    {
        if (!pGroup)
            pGroup = m_pPropertyCtrl->InsertGroupItem(strGroupName.c_str(), _T("Id样式"), UITVI_ROOT, UITVI_LAST, 0);
...
void LayoutPropertyHandler::AttachObject_ClassAttrib(ISkinRes* hSkin, IObject* pObj, const String& strClassStyle)
{
    ...
    for (int i = 0; i < nCount; i++)
    {
        ...
        IPropertyCtrlGroupItem* pGroup = m_pPropertyCtrl->InsertGroupItem(strGroupName.c_str(), _T("Class样式"), UITVI_ROOT, UITVI_LAST, 0);
