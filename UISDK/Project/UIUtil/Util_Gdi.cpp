...
BOOL  PreMultiAlpha(HBITMAP hBitmap, RECT* lprc, BOOL bTopDownDib)
{
    DIBSECTION  dibsec;
    if (GetObject(hBitmap, sizeof(dibsec), &dibsec) < sizeof(dibsec))
        return FALSE;

    if (dibsec.dsBm.bmBitsPixel != 32)
        return FALSE;

    RECT rcDest;
    if (lprc)
    {
        RECT rcBitmap = {0, 0, dibsec.dsBm.bmWidth, dibsec.dsBm.bmHeight};
        if (FALSE == ::IntersectRect(&rcDest, &rcBitmap, lprc))
            return FALSE;
    }
    else
    {
        SetRect(&rcDest, 0, 0, dibsec.dsBm.bmWidth, dibsec.dsBm.bmHeight);
    }

    BYTE* pBits = (BYTE*)dibsec.dsBm.bmBits;
    int nPitch = dibsec.dsBm.bmWidthBytes;
    if (!bTopDownDib)
    {
        pBits = LPBYTE( pBits )+((dibsec.dsBm.bmHeight-1)*nPitch);
        nPitch = -nPitch;
    }
    ...
    for (int y = rcDest.top; y < rcDest.bottom; y++)
    {
        for (int x = rcDest.left; x < rcDest.right; x++)
        {
            ...
            a = pBits[i+3];
            pBits[i]   = pBits[i]  * a >> 8;
            pBits[i+1] = pBits[i+1]* a >> 8;
            pBits[i+2] = pBits[i+2]* a >> 8;
        }
        pBits += nPitch;
    }
    return TRUE;
