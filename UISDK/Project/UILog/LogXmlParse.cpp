...
#define XML_LOGFLAG_FILE          _T("file")
#define XML_LOGFLAG_FUNCTION      _T("function")
#define XML_LOGFLAG_LINE          _T("line")
...
//			_stscanf( strLogFlag.c_str(), _T("0x%x"), &uLogFlag);
			...
			for (int i = 0; i < nSize; i++)
			{
				...
				else if (vecFlags[i] == XML_LOGFLAG_FILE)
				{
					uLogFlag |= LOG_FLAG_FILE; 
				}
				else if (vecFlags[i] == XML_LOGFLAG_FUNCTION)
				{
					uLogFlag |= LOG_FLAG_FUNCTION;
				}
				else if (vecFlags[i] == XML_LOGFLAG_LINE)
				{
					uLogFlag |= LOG_FLAG_LINE;
				}
			}
