...
void  Color::SetHSL(HSL& hsl)
{
    ...
    if (hsl.saturation == 0)
    {
        r = static_cast<byte>(round(255.0 * hsl.lightness * 255.0));
        g = static_cast<byte>(round(255.0 * hsl.lightness * 255.0));
        b = static_cast<byte>(round(255.0 * hsl.lightness * 255.0));
    }
    else
    {
        double  temp2 = 0;
