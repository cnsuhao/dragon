		...
		VARIANT *pvarRole, BOOL& bHandled)
{
	...
	OBJ_TYPE eObjType = pObj->GetObjectType();
	...
	{
		pvarRole->vt = VT_I4;
		CONTROL_TYPE eCtrlType = pObj->GetObjectExtentType();
		...
		VARIANT *pvarState, BOOL& bHandled)
{
	...
	int nStateBit = pObj->GetStateBit();

	if (nStateBit & (OSB_CHECKED|OSB_RADIOCHECKED))
