...
void  QuadPerspectiveTextureMapping::do_bilinear(POINT*  ptOffset, RECT* prcClip)
{
    ...
    // 转成目标区域
    Quad  quadDest = m_quad; // m_quad是纹理区域，转成目标区域还得去掉边界偏移（如1,1对应0,0）
    quadDest.Offset(ptOffset->x-BILINEAR_EDGE_PADDING, ptOffset->y-BILINEAR_EDGE_PADDING);

	// 接下来用目标位置取纹理位置时，目标位置的(0,0)，应该用偏移后的位置(1,1)取（例如转换为identify情况）。
	ptOffset->x -= BILINEAR_EDGE_PADDING; // 用-= 是因为下面调用perspective_transform时，用的是X-ptOffset->x
	ptOffset->y -= BILINEAR_EDGE_PADDING;
    ...
    for (int X = 0, Y = nMinY; Y <= nMaxY; Y++)  // quad的边界值也要计算，在计算时已经是索引范围
    {
        for (X = nMinX; X <= nMaxX; X++)
        {
            ...
            //inline DWORD& Bilinear(BYTE* pBits, const int& nPitch, const int& x, const int& y, const float& fx, const float& fy)
            {
                ...
                int nPos = X << 2;
                byte nAlphaSub = 255 - ColorResult.a;
