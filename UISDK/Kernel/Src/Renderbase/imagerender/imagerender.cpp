...
#include "UISDK\Kernel\Src\Base\Attribute\rect_attribute.h"
#include "UISDK\Kernel\Src\Base\Attribute\9region_attribute.h"
...
void  ImageRender::OnSerialize(SERIALIZEDATA* pData)
{
	...
	// 绘制透明度
	s.AddLong(XML_RENDER_IMAGE_ALPHA, m_nAlpha);
	...
	else
	{
		if (m_bIsAnimate)
		{
			...
			else
			{
                ...
                {
                    DrawIndexWidthAlpha(pRenderTarget, prc, 4, (byte)(255-m_nCurrentAlpha));
                    DrawIndexWidthAlpha(pRenderTarget, prc, 5, (byte)m_nCurrentAlpha);
                }
                else   // pushbutton/defaultbutton
                {
                    bool bDefault = (nRenderState & RENDER_STATE_DEFAULT)?true:false;

                    DrawIndexWidthAlpha(pRenderTarget, prc, bDefault?4:0, (byte)(255-m_nCurrentAlpha)); 
                    DrawIndexWidthAlpha(pRenderTarget, prc, 1, (byte)m_nCurrentAlpha);
                }
			}
		}
		else
