...
{
	RenderLayer*  pLayer = m_pWindow->GetSelfRenderLayer2();
    ...
    bf.SourceConstantAlpha = (byte)m_nAlphaValue;         // 0~255 透明度
    // 
...
{
    // ?? 不调用这句话，窗口最小化后再还原RGN还是最小化的样子，为什么
    SetWindowRgn(GetHWND(), NULL, FALSE);
}

void  LayeredWindowWrap::UpdateLayeredCaptionWindowRgn()
{
	HWND hWnd = GetHWND();
