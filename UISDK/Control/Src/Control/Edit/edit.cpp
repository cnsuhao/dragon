...
#include "UISDK\Control\Src\Control\RichEdit\TextFilter\digitfilter.h"
#include "UISDK\Control\Src\Control\RichEdit\textfilter\uintfilter.h"
	...
	if (bInsertOrOverride)
	{
		...
		if (m_nMaxChar >= 0)
		{
			...
			if (0 == nRemain)
			{
				if (bUpdate)
				{
					this->Fire_Text_Changed(false);  // 由于删除了选区导致text changed
				}
				return;
			}
...
TextFilterResult EditData::FilterString(LPCTSTR szSrc, String& strDest)
{
	if (m_pTextFilter)
	{
		...
		SETSTRING(strDest, m_pTextFilter->StringFilter(szSrc, eResult));
...
// 
void Edit::SetText(LPCTSTR szText, bool bUpdate)
{
	setText(szText, true, bUpdate);
}

void  Edit::SetTextNoFilter(LPCTSTR szText, bool bUpdate)
{
	setText(szText, false, bUpdate);
}
...
void  Edit::OnSerialize(SERIALIZEDATA* pData)
{
	DO_PARENT_PROCESS(IEdit, IControl);
    {
        ...
        s.AddString(XML_EDIT_COLOR, this, 
            memfun_cast<pfnStringSetter>(&Edit::LoadColor),
            memfun_cast<pfnStringGetter>(&Edit::SaveColor));
        s.AddString(XML_EDIT_SELECT_COLOR, this, 
            memfun_cast<pfnStringSetter>(&Edit::LoadSelectColor),
            memfun_cast<pfnStringGetter>(&Edit::SaveSelectColor));
        s.AddString(XML_EDIT_SELECT_BK_COLOR, this, 
            memfun_cast<pfnStringSetter>(&Edit::LoadSelectBkgColor),
            memfun_cast<pfnStringGetter>(&Edit::SaveSelectBkgColor));
        ...
        s.AddLong(XML_EDIT_CARET_HEIGHT, m_nCaretHeight);
    }
}
...
IRenderFont*  Edit::GetRenderFont()
{
    ITextRenderBase* pTextRender = m_pIEdit->GetTextRender();
    if (pTextRender)
        return pTextRender->GetRenderFont();

     return m_pIEdit->GetRenderFont();
}
...
long  Edit::GetCaretHeight()
{
    ...
    IRenderFont* pFont = GetRenderFont();
    if (!pFont)
        return 0;
...
void Edit::OnPaint(IRenderTarget* pRenderTarget, RenderContext* pContext)
{
	...
	this->DrawNormal(pRenderTarget, pContext);
    ...
    if (pRenderTarget->IsRenderAlphaChannel())
    {
        ...
        OffsetRect(&rc, 
                pContext->m_ptBufferOffset.x, 
                pContext->m_ptBufferOffset.y);  // 缓存还有偏移
        data.lprc = &rc;
