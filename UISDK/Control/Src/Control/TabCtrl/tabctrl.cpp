...
HRESULT  TabCtrl::UIParseNewItem(IUIElement* pChildElem)
{
    ...
    if (pHead)
    {
        ...
        if (pChildElem)
        {
            pIObjHead = pLayoutMgr->ParseElement(pChildElem, m_pPanelHead);
    ...
    if (pContent)
    {
        ...
        if (pChildElem)
        {
            pIObjContent = pLayoutMgr->ParseElement(pChildElem, m_pPanelContent);
            SAFE_RELEASE(pChildElem);
        }
...
void  TabCtrlBase::SetAttribute(IMapAttribute* pMapAttr, bool bReload)
{
    ...
    LPCTSTR  szText = pMapAttr->GetAttr(XML_TABCTRL_HEAD_LAYOUT_TYPE, true);
    if (szText)
    {
...
void  TabCtrlBase::OnSize(UINT nType, int cx, int cy)
{
    ...
    CRect  rcClient;
    m_pITabCtrlBase->GetClientRectInObject(&rcClient);
    cx = rcClient.Width();
    cy = rcClient.Height();
...
void  TabCtrlBase::OnBnClicked(IMessage*  pMsgFrom)
{
    ...
    if (NULL == pItem)
    {
        UI_LOG_WARN(_T("FindItemByBtn Failed."));
        return ;
    }
...
void  TabCtrl::AddItem(LPCTSTR  szText, IObject* pContentObj)
{
	...
	SERIALIZEDATA data = {0};
    data.pUIApplication = m_pITabCtrl->GetUIApplication();
	data.pMapAttrib = pMapAttrib;
	data.nFlags = SERIALIZEFLAG_LOAD;
