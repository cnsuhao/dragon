...
void IEWrap::SetAttribute(IMapAttribute* pMatAttrib, bool bReload)
{
	...
	if (!bReload)
	{
		LPCTSTR szText = pMatAttrib->GetAttr(XML_IE_URL, true);
		if (szText)
		{
			if (FAILED(Navigate(szText)))
			{
				UI_LOG_WARN(_T("Navigate failed. url=%s"), szText);
			}
		}
	}
}
void IEWrap::CreateControl()
{
	...
	do
	{
		...
		{
			UI_LOG_WARN(_T("Create WebBrowser ActiveX Control Failed."));
			break;
		}
...
HRESULT  IEWrap::Navigate(LPCTSTR szUrl)
{
	if (!szUrl)
		return E_INVALIDARG;
...
{   
    m_bNeedFixAlpha = false;
    if (pRenderTarget->IsRenderAlphaChannel() && m_hWndIE)   // 在分层拖拽窗口过程中由于忽略了WS_CLIPCHILDREN样式，导致IE在拖拽中不能显示，造成闪烁，在这里画上去
    {
        WINDOW_TRANSPARENT_TYPE  eMode = (WINDOW_TRANSPARENT_TYPE)
                UISendMessage(m_pIIEWrap->GetWindowObject(), UI_WM_GET_WINDOW_TRANSPARENT_MODE, 0);

        if (WINDOW_TRANSPARENT_TYPE_LAYERED == eMode ||
            WINDOW_TRANSPARENT_TYPE_AERO == eMode)
        {
...
//         HDC hDCDst = GetDC(m_pIIEWrap->GetHWND());
//         if (hDCDst)
//         {
//             ReleaseDC(m_pIIEWrap->GetHWND(), hDCDst);
//         }
        return 0;
    }
