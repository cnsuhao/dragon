...
bool  FontCache::GetFont(RichTextCharFormat* pcf)
{
    if (!pcf)
        return false;
    ...
    for (uint i = 0; i < nSize; i++)
    {
		...
		if (lf.lfStrikeOut != (pcf->bDeleteline?1:0))
			continue;
        if (lf.lfHeight != Util::FontSize2Height(pcf->lFontSize))
            continue;
...
HFONT  FontCache::CreateFont(RichTextCharFormat& cf)
{
    HFONT hFont = ::CreateFont(
        Util::FontSize2Height(cf.lFontSize),  // nHeight
        0,                         // nWidth
        0,                         // nEscapement
        0,                         // nOrientation
