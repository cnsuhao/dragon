...
{
    DO_PARENT_PROCESS(ICompoundButton, IButtonBase);
	SAFE_RELEASE(m_pLayout);
}

void CompoundButton::OnSerialize(SERIALIZEDATA* pData)
{
    DO_PARENT_PROCESS(ICompoundButton, IButtonBase);

    if (pData->IsLoad())
    {
        LPCTSTR szText = pData->pMapAttrib->GetAttr(XML_LAYOUT, true);
        m_pICompoundButton->GetUIApplication()->CreateLayoutByName((BSTR)szText, m_pICompoundButton, true, &m_pLayout);
        if (m_pLayout)
        {
            m_pLayout->Serialize(pData);
        }
    }
}
