...
void  ProgressBarBkgndThemeRender::OnInit()
{
    SetMsgHandled(FALSE);
    if (GetObject())
    {
        m_pIProgressCtrl = (IProgressBar*)GetObject()->QueryInterface(uiiidof(IProgressBar));
    }
}

void  ProgressBarBkgndThemeRender::DrawState(RENDERBASE_DRAWSTATE* pDrawStruct)
{
    IRenderTarget* pRenderTarget = pDrawStruct->pRenderTarget;
    ...
    if (hTheme)
    {
        ...
        if (m_pIProgressCtrl)
        {
            PROGRESSBAR_SCROLL_DIRECTION_TYPE eType = m_pIProgressCtrl->GetDirectionType();
            if (PROGRESSBAR_SCROLL_BOTTOM_2_TOP == eType || PROGRESSBAR_SCROLL_TOP_2_BOTTOM == eType)
            {
                iPartId = PP_BARVERT;
            }
        }
...
void  ProgressBarForegndThemeRender::OnInit()
{
    SetMsgHandled(FALSE);
    if (GetObject())
    {
...
void  ProgressBarForegndThemeRender::DrawState(RENDERBASE_DRAWSTATE* pDrawStruct)
{
    IRenderTarget* pRenderTarget = pDrawStruct->pRenderTarget;
    int& nState = pDrawStruct->nState;
    ...
    else
    {
        ...
        PROGRESSBAR_SCROLL_DIRECTION_TYPE eType = PROGRESSBAR_SCROLL_LEFT_2_RIGHT;
        if (m_pIProgressCtrl)
        {
...
void ComboboxBkThemeRender::DrawNormal( IRenderTarget* pRenderTarget, const CRect* prc )
{
	...
	{
		if (static_cast<IComboBox*>(GetObject())->IsReadOnly())
		{
			DrawEdge(hDC, (RECT*)prc, EDGE_SUNKEN, BF_RECT|BF_MIDDLE);
		}
		else
    ...
    if (hTheme)
    {
        ...
        if (m_pIMenu)
        {
            ...
            if (FAILED(DrawThemeBackground(hTheme, hDC, MENU_POPUPGUTTER, 1, (RECT*)&rcIconGutter, 0)))
                UI_LOG_WARN(_T("DrawThemeBackground MENU_POPUPGUTTER failed."));
        }
    }
    else
    {
...
void  MenuTextThemeRender::OnSerialize(SERIALIZEDATA* pData)
{
	{
		AttributeSerializerWrap s(pData, TEXT("MenuTextThemeRender"));
		static_cast<IThemeTextRenderBase*>(this)->Serialize(&s);

		s.AddString(XML_TEXTRENDER_FONT, this, 
			memfun_cast<pfnStringSetter>(&MenuTextThemeRender::LoadFont),
			memfun_cast<pfnStringGetter>(&MenuTextThemeRender::SaveFont));
	}
...
void  MenuTextThemeRender::LoadFont(LPCTSTR szFont)
{
	_LoadFont(szFont, m_pRenderFont);
}
LPCTSTR  MenuTextThemeRender::SaveFont()
{
...
void  ListViewThemeTextRender::LoadFont(LPCTSTR szFont)
{
	_LoadFont(szFont, m_pRenderFont);
}
LPCTSTR  ListViewThemeTextRender::SaveFont()
{
	return _SaveFont(m_pRenderFont);
}
void  ListViewThemeTextRender::OnSerialize(SERIALIZEDATA* pData)
{
	{
		AttributeSerializerWrap s(pData, TEXT("ListViewThemeTextRender"));
		static_cast<IThemeTextRenderBase*>(this)->Serialize(&s);

		s.AddString(XML_TEXTRENDER_FONT, this, 
			memfun_cast<pfnStringSetter>(&ListViewThemeTextRender::LoadFont),
			memfun_cast<pfnStringGetter>(&ListViewThemeTextRender::SaveFont));
	}
