...
Gauss_Rand (DBL c, DBL A, DBL S)
{
	...
	y = (DBL)(LRAND () / MAXRAND);
	y = (DBL)(A * (1.0 - exp (-y * y * S)) / (1.0 - exp (-S)));
	if (NRAND (2))
		return (c + y);
	return (c - y);
}
...
{
	while (i--) {
		Cur->c_x = Gauss_Rand (0.0f, .8f, 4.0f);
		Cur->c_y = Gauss_Rand (0.0f, .8f, 4.0f);
		Cur->r = Gauss_Rand (F->r_mean, F->dr_mean, 3.0f);
		Cur->r2 = Half_Gauss_Rand (0.0, F->dr2_mean, 2.0f);
		Cur->A = Gauss_Rand (0.0f, 360.0f, 4.0f) * (M_PI / 180.0f);
		Cur->A2 = Gauss_Rand (0.0f, 360.0f, 4.0f) * (M_PI / 180.0f);
		Cur++;
	}
}
