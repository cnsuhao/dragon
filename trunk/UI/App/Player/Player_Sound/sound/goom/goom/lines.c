...
genline (int id, float param, GMUnitPointer * l, int rx, int ry)
{
	...
	case GML_CIRCLE:
		for (i = 0; i < 512; i++) {
			...
			l[i].angle = 2.0f * (float)M_PI * (float) i / 512.0f;
			cosa = (float)(param * cos (l[i].angle));
			sina = (float)(param * sin (l[i].angle));
...
goom_lines_draw (GMLine * line, gint16 data[512], unsigned int *p)
{
	if (line != NULL) {
		...
		float   cosa = (float)cos (pt->angle) / 1000.0f;
		float   sina = (float)sin (pt->angle) / 1000.0f;
