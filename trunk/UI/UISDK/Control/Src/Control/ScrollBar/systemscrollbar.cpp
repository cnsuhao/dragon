...
void  SystemScrollBar::OnInitialize()
{
	...
	IObject* pObj = m_pISystemScrollBar->FindChildObject(SCROLLBAR_THUMB_ID);
	if (pObj)
	...
	m_pBtnThumb->SetButtonStyle( isVScrollBar ? 
		BUTTON_STYLE_VSCROLLTHUMB:BUTTON_STYLE_HSCROLLTHUMB);

	m_pBtnThumb->SetTabstop(false);

	pObj = m_pISystemScrollBar->FindChildObject(isVScrollBar?
			SCROLLBAR_UP_ID:SCROLLBAR_LEFT_ID);
	if (pObj)
		m_pBtnLineUpLeft = (IButton*)pObj->QueryInterface(uiiidof(IButton));
	...
	if (m_pBtnLineUpLeft)
	{
		...
		m_pBtnLineUpLeft->SetButtonStyle( 
			BUTTON_STYLE_SCROLLLINEUP);  // 先取个默认值
	...
	if (pObj)
		m_pBtnLineDownRight = (IButton*)pObj->QueryInterface(uiiidof(IButton));
	...
	if (m_pBtnLineDownRight)
	{
		...
		m_pBtnLineDownRight->SetButtonStyle(
			BUTTON_STYLE_SCROLLLINEDOWN);  // 先取个默认值
	...
	if (m_bHoverVisible)
	{
		m_pISystemScrollBar->SetRenderLayer(true);
		IRenderLayer* p = m_pISystemScrollBar->GetSelfRenderLayer2();
		if (p)
		{
			p->SetOpacity(0);
...
void  SystemScrollBar::CreateHoverAnimate()
{
    ...
    IAnimateManager* pAnimateMgr = 
        m_pISystemScrollBar->GetUIApplication()->GetAnimateMgr();
    IStoryboard* pHoverAnimate = pAnimateMgr->CreateStoryboard(m_pISystemScrollBar, STORYBOARD_ID_HOVER_ANIMATE, 0, 0);

    IIntTimeline* pTimeline = pHoverAnimate->CreateIntTimeline(0);
    if (m_bHover)
    {
        // 这里不使用0->255
        // 有可能当前的动画是取消了255-0的动画，当前的alpha值可能是不确定的。
        // 为了避免闪烁，动画的起始值应该设置为当前opacity
        pTimeline->SetLinerParam1(
            m_pISystemScrollBar->GetSelfRenderLayer2()->GetOpacity(), 255, 200);
    }
    else
    {
        pTimeline->SetLinerParam1(
            m_pISystemScrollBar->GetSelfRenderLayer2()->GetOpacity(), 0, 200);
    }
...
void  SystemScrollBar::DestroyHoverAnimate()
{
    IAnimateManager* pAnimateMgr = 
        m_pISystemScrollBar->GetUIApplication()->GetAnimateMgr();
    pAnimateMgr->RemoveStoryboard(m_pISystemScrollBar, STORYBOARD_ID_HOVER_ANIMATE);
}
...
LRESULT  SystemScrollBar::OnHoverChanged(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    if (GetDirType() == VSCROLLBAR && m_pMgrScrollBar->GetHScrollBar())
    {
        ...
        ISystemHScrollBar* pHScrollBar = (ISystemHScrollBar*)p->QueryInterface(uiiidof(ISystemHScrollBar));
        if (pHScrollBar)
        {
    ...
    else if (GetDirType() == HSCROLLBAR && m_pMgrScrollBar->GetVScrollBar())
    {
        ...
        ISystemVScrollBar* pVScrollBar = (ISystemVScrollBar*)p->QueryInterface(uiiidof(ISystemVScrollBar));
        if (pVScrollBar)
        {
...
void  SystemScrollBar::OnAnimateTick(int nCount, UI::IStoryboard** ppArray)
{
    for (int i = 0; i < nCount; i++)
    {
        IStoryboard* pStoryboard = ppArray[i];
        if (pStoryboard->GetId() == STORYBOARD_ID_HOVER_ANIMATE)
        {
            int nValue = 255;
            pStoryboard->GetTimeline(0)->GetCurrentValue(&nValue);

            m_pISystemScrollBar->GetSelfRenderLayer2()->SetOpacity(nValue);
            m_pISystemScrollBar->UpdateObject();
        }
    }
}
...
void  SystemScrollBar::CalcScrollBarVisible()
{
	...
	if (nPage >= nRange)
	{
		...
		case SCROLLBAR_VISIBLE_SHOW_ALWAYS:
            m_pISystemScrollBar->SetVisible(true, false, false);
