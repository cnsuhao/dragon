...
void  MenuPopupItemShareData::SetAttribute(IMapAttribute* pMapAttrib, bool bReload)
{
    ...
    LPCTSTR szText = pMapAttrib->GetAttr(XML_MENU_TRAIANGLE_RENDER_PREFIX XML_RENDER_TYPE, true);
    if (szText)
    {
        SAFE_RELEASE(m_pPopupTriangleRender);
        ...
        if (m_pPopupTriangleRender)
        {
            ...
            data.pUIApplication = m_pListCtrl->GetUIApplication();
            data.pMapAttrib = pMapAttrib;
            data.szPrefix = XML_MENU_TRAIANGLE_RENDER_PREFIX;
            data.nFlags = SERIALIZEFLAG_LOAD|SERIALIZEFLAG_LOAD_ERASEATTR;
            m_pPopupTriangleRender->Serialize(&data);
        }
        else
        {
            UI_LOG_WARN( _T("Invalid %s=\"%s\""), XML_MENU_TRAIANGLE_RENDER_PREFIX XML_RENDER_TYPE, szText);
        }
    }
