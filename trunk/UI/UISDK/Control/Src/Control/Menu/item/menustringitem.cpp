...
void  MenuStringItemShareData::SetAttribute(IMapAttribute* pMapAttrib, bool bReload)
{
    ...
    LPCTSTR szText = pMapAttrib->GetAttr(XML_MENU_RADIO_ICON_RENDER_PREFIX XML_RENDER_TYPE, true);
    if (szText)
    {
        SAFE_RELEASE(m_pRadioIconRender);
        ...
        if (m_pRadioIconRender)
        {
            ...
            data.szPrefix = XML_MENU_RADIO_ICON_RENDER_PREFIX;
            data.nFlags = SERIALIZEFLAG_LOAD|SERIALIZEFLAG_LOAD_ERASEATTR;
            m_pRadioIconRender->Serialize(&data);
        }
        else
        {
            UI_LOG_WARN( _T("Invalid %s=\"%s\""), XML_MENU_RADIO_ICON_RENDER_PREFIX XML_RENDER_TYPE, szText);
        }
    }
    ...
    if (szText)
    {
        ...
        if (m_pCheckIconRender)
        {
            ...
            data.pUIApplication = pUIApplication;
            data.pMapAttrib = pMapAttrib;
            data.szPrefix = XML_MENU_CHECK_ICON_RENDER_PREFIX;
            data.nFlags = SERIALIZEFLAG_LOAD|SERIALIZEFLAG_LOAD_ERASEATTR;
            m_pCheckIconRender->Serialize(&data);
        }
        else
        {
            UI_LOG_WARN( _T("Invalid %s=\"%s\""), XML_MENU_CHECK_ICON_RENDER_PREFIX XML_RENDER_TYPE, szText);
        }
    }
    ...
    if (szText)
    {
        ...
        if (m_pRadioCheckIconBkRender)
        {
            ...
            data.pUIApplication = pUIApplication;
            data.pMapAttrib = pMapAttrib;
            data.szPrefix = XML_MENU_RADIOCHECK_ICONBK_RENDER_PREFIX;
            data.nFlags = SERIALIZEFLAG_LOAD|SERIALIZEFLAG_LOAD_ERASEATTR;
            m_pRadioCheckIconBkRender->Serialize(&data);
        }
        else
        {
            UI_LOG_WARN( _T("Invalid %s=\"%s\""), XML_MENU_RADIOCHECK_ICONBK_RENDER_PREFIX XML_RENDER_TYPE, szText);
        }
    }
