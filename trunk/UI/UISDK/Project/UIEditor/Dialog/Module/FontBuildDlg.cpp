...
void  CFontViewDlg::OnBtnAdd(UINT uNotifyCode, int nID, CWindow wndCtl)
{
	...
	if( IDOK == dlg.DoModal() )
	{
		...
		m_listctrl.SetItemText(nCount,7,szText);
 		...
 		// 保存数据用于自绘
		IFontRes* pFontRes = m_pSkin->GetFontRes();
 		IRenderFont* pRenderFont = NULL;
		if (pFontRes)
		{
			pFontRes->GetFont((BSTR)dlg.m_strId.c_str(), GRAPHICS_RENDER_LIBRARY_TYPE_GDI, &pRenderFont);
...
void  CFontViewDlg::OnBtnRemove(UINT uNotifyCode, int nID, CWindow wndCtl)
{
	...
	IFontManager* pFontMgr = m_pSkin->GetFontManager();
	if (NULL == pFontMgr)
		return ;
...
bool CFontViewDlg::Reload()
{
	...
	// 获取该皮肤中的所有图片
	long nCount = pFontRes->GetFontCount();
	for (long i = 0; i < nCount; i++)
	{
		IFontResItem*  pInfo = NULL;
		if (SUCCEEDED(pFontRes->GetFontResItem(i, &pInfo)) && pInfo!=NULL)
		{
...
LRESULT  CFontViewDlg::OnItemDBClick(LPNMHDR pnmh)
{
	...
	if( IDOK == dlg.DoModal() )
	{
		...
		if (pFontRes)
		{
			pFontRes->GetFont((BSTR)dlg.m_strId.c_str(), GRAPHICS_RENDER_LIBRARY_TYPE_GDI, &pFont);
