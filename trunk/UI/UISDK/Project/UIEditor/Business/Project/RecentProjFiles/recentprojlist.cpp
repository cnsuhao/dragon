...
RecentProjItem* CRecentProjList::GetItem(const TCHAR* szProjPath)
{
	vector<RecentProjItem*>::iterator  iter = m_arrRecentProj.begin();
	vector<RecentProjItem*>::iterator  iterEnd = m_arrRecentProj.end();
	for (; iter != iterEnd; iter++)
...
bool  CRecentProjList::Save()
{
	...
	{
		if (false == Util::CreateEmptyXmlFileW(szConfigXmlPath, _T("config"), NULL))
			return false;
	...
	do 
	{
		...
		if (xml.FindElem(_T("recent_project_list")))  
		{
			xml.RemoveElem();
		}
...
bool CRecentProjList::Add(const TCHAR* szProjPath)
{
	time_t ltime = 0;
	time( &ltime );
...
bool CRecentProjList::Remove(const TCHAR* szProjPath)
{
	return false;
}
bool CRecentProjList::Load()
{
	...
	do 
	{
		...
		if (false == xml.FindElem(_T("recent_project_list")))  break;
		if (false == xml.IntoElem()) break;
		...
		while(xml.FindElem())
		{
			...
			Util::GetPathFileName(pItem->szFilePath, pItem->szFileName);
