...
#define TIMELIEN_FLAG_REVERSING    0x0010   // 表示正在反向进行动画
    ...
    public:
        ...
        virtual bool x_OnTick()
        {
            ...
            case TT_BY_MS:
                nTimeElapse = (int)m_stopWath.Now();
                break;

            case TT_BY_SECOND:
                nTimeElapse = (int)((m_stopWath.Now())/1000);
            }
...
//             OutputDebugString(szText);
	...
	protected:
		...
		void  _ResetTime()
		{
			...
			m_nFlags &= ~(TIMELIEN_FLAG_REVERSING|TIMELINE_FLAG_NEED_RESET|TIMELINE_FLAG_FINISH);
		}
    ...
    protected:
        ...
        StopWatch   m_stopWath;   // 计时 记录动画开始时的time tick，当时间到达m_nBeginTime+m_nDuretion时，动画结束。
