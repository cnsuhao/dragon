...
#include "UISDK\Kernel\Src\Util\Gesture\gesturehelper.h"
...
void  WindowMouseMgr::SetUIApplication(IUIApplication* p)
{
    m_pUIApplication = p;
}
...
IUIApplication*  WindowMouseMgr::GetUIApplication()
{
	...
	return m_oWindow.GetIUIApplication();
}
...
long WindowMouseMgr::OnMouseWheel(WPARAM w, LPARAM l)
{
    ...
    if (m_pObjPress)
    {
        BOOL  bHandled = UISendMessage(m_pObjPress->GetIMessage(), UI_WM_MOUSEWHEEL_REQ, w, l);
        if (bHandled)
            return 0;
    }
