...
void  StyleResItem::SetXmlElement(IUIElement* p)
{
	SAFE_RELEASE(m_pXmlElement);
	m_pXmlElement = p;
	if (m_pXmlElement)
...
IUIElement*  StyleResItem::GetXmlElement()
{
	return m_pXmlElement;
}
bool  StyleResItem::GetXmlElement2(IUIElement** pp)
{
	if (!pp || !m_pXmlElement)
		return false;
...
bool  StyleRes::FilterStyle(LPCTSTR szTagName, LPCTSTR szStyleClass, LPCTSTR szID, IListAttribute* pListAttribte)
{
    ...
    {
        pListAttribte->AddAttr(szKey, EMPTYTEXT);
    }
    pStyleAttr->EndEnum();
