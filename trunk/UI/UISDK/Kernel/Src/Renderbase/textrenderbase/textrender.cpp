...
// 		->AddFlag(DT_LEFT, XML_TEXTRENDER_ALIGN_LEFT)
		...
		->AddFlag(DT_WORDBREAK, XML_TEXTRENDER_ALIGN_MULTILINE)
...
void  TextRenderBase::_LoadFont(LPCTSTR szFontId, IRenderFont*& pRenderFont)
{
    ...
    pFontRes->GetFont(szFontId, m_pObject->GetGraphicsRenderLibraryType(), &pRenderFont);
}
...
void  TextRenderBase::_LoadDefalutFont(IRenderFont*& pRenderFont)
{
    IRenderFont* pFont = m_pObject->GetRenderFont();
    if (pFont)
    {
        pRenderFont = m_pObject->GetRenderFont();
        if (pRenderFont)
            pRenderFont->AddRef();
    }
    else
    {
        ...
        if (pFontRes && m_pObject)
            pFontRes->GetDefaultFont(m_pObject->GetGraphicsRenderLibraryType(), &pRenderFont);
    }    
}
