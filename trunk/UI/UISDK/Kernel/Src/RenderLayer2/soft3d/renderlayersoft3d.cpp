...
void  RenderLayer2Soft3D::Rotate(int x, int y, int z)
{
    // 计算时用的居然是相反的，先在这里减去360度吧
    Mat_Rotate_4X4(&m_matWorld, 360-x, 360-y, 360-z);
...
void  RenderLayer2Soft3D::Update3dPos()
{
    ...
    {
        m_pTexturemapping->m_quad.GetBoundRect(&m_rcInvalid);
...
void  RenderLayer2Soft3D::TransformPoint(POINT* pptLayer, __out POINT* pptTransform)
{
    ...
    pptTransform->x = round(fx) - ptBufferOffset.x;
    pptTransform->y = round(fy) - ptBufferOffset.y;
}
...
bool  RenderLayer2Soft3D::HitTest(POINT*  pptParent, POINT*  ptInChild)
{
    ...
    // 先判断是否在bound区域
    CRect  rcBound;
    m_pTexturemapping->m_quad.GetBoundRect(&rcBound);
