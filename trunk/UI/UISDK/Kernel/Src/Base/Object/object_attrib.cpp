...
void  Object::OnSerialize(SERIALIZEDATA* pData)
{
...

    {
		...
		s.AddLong(XML_ZORDER, m_lzOrder);  // z序 (注：默认在从xml加载之后，AddChild之前会先解析一次)
...
//      s.AddRenderBase(XML_BACKGND_RENDER_PREFIX, XML_RENDER_TYPE, m_pIObject, m_pBkgndRender);  // 设置背景渲染器
//      s.AddRenderBase(XML_FOREGND_RENDER_PREFIX, XML_RENDER_TYPE, m_pIObject, m_pForegndRender); // 设置前景绘制
