...
{ 
    ...
    if (rcItemExpand.PtInRect(ptClient))   
    {
        ...
        m_pINormalTreeItem->GetIListCtrlBase()->SetInvalidateAllItems();
    }
    else
    {
...
void  NormalTreeItem::GetItemExpandIconRect(CRect* prc)
{
    ...
    prc->left = m_pINormalTreeItem->GetIListCtrlBase()->GetChildNodeIndent() * m_pINormalTreeItem->CalcDepth();
...
void  NormalTreeItem::GetDesiredSize(SIZE* pSize)
{
    ...
    pSize->cx += pListCtrl->GetChildNodeIndent()* m_pINormalTreeItem->CalcDepth();
