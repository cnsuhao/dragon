...
//		lparam: NA.
//
#define UI_WM_REDRAWOBJECT  147011821
...
//  lparam : NA.
//  Return: HANDLED/NOTHANDED;
//
#define UI_WM_OWNERDRAW  157031548
...
struct SERIALIZEDATA
{
	union
    {
        ...
        UI::IAttributeEditorProxy*  pAttributeEditorProxy;  // editor [in]
	};

    UI::IUIApplication*  pUIApplication;
...
#define  UI_WM_CREATEBYEDITOR  154061549

// void  OnCreateByEditor(CREATEBYEDITORDATA* pData);
#define  UIMSG_WM_CREATEBYEDITOR(func)                \
	if (uMsg == UI_WM_CREATEBYEDITOR)                 \
    {                                                 \
        SetMsgHandled(TRUE);                          \
...
//
#define UI_WM_INITIALIZE  155051209
#define UI_WM_INITIALIZE2 155051210
...
// void  OnInitialize2();
#define UIMSG_WM_INITIALIZE2(func)                    \
    if (uMsg == UI_WM_INITIALIZE2)                    \
    {                                                 \
        SetMsgHandled(TRUE);                          \
        func();                                       \
        if (IsMsgHandled())                           \
...
//
#define UI_WM_MOUSEWHEEL_REQ  142841001
...
//
#define  UI_WM_SHOW_TOOLTIP  149150933
...
//
#define UI_WM_GET_TOOLTIPINFO 149150934
...
// 需要控件有post_paint样式
#define UI_WM_POSTPAINT        147281159
// void OnPostPaint(IRenderTarget* pRenderTarget)
#define UIMSG_WM_POSTPAINT(func) \
	if (uMsg == UI_WM_POSTPAINT) \
	{ \
		SetMsgHandled(TRUE); \
        func((UI::IRenderTarget*)wParam); \
		lResult = 0; \
...
#define UIMSG_WM_SETATTRIBUTE(func)                   \
    ...
    {                                                 \
        SetMsgHandled(TRUE);                          \
        UI::SERIALIZEDATA* pData = (UI::SERIALIZEDATA*)wParam; \
		if (pData->nFlags&SERIALIZEFLAG_LOAD)           \
		{ \
			func((UI::IMapAttribute*)pData->pMapAttrib, pData->IsReload()); \
			if (IsMsgHandled())                           \
	            return TRUE;                              \
		} \
    }
