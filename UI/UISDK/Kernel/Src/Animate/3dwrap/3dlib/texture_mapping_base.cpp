...
void  TextureMappingBase::GetTexturePadding(RECT* prc)
{
    ...
    {
        SetRect(prc,
			BILINEAR_EDGE_PADDING,
			BILINEAR_EDGE_PADDING,
			BILINEAR_EDGE_PADDING,
			BILINEAR_EDGE_PADDING);  // 需要一px边界哨兵
    }
    else
    {
        SetRect(prc, 0, 0, 0, 0);
...
void TextureMappingBase::SetSrcBuffer(ImageData* pBuffer)
{
	...
	{
		//  为纹理添加四周一px边界哨兵
		int nWidth = pBuffer->m_nWidth + 2*BILINEAR_EDGE_PADDING;
		int nHeight = pBuffer->m_nHeight + 2*BILINEAR_EDGE_PADDING;

		m_imageTexture.Create(nWidth, -nHeight, 32, Image::createAlphaChannel);
		byte* pDstBits = (byte*)m_imageTexture.GetBits();
		
		m_texture.m_nbpp = 32;
		m_texture.m_nWidth = nWidth;
		m_texture.m_nHeight = nHeight;
		m_texture.m_nStride = m_imageTexture.GetPitch();
		m_texture.m_pScan0 = (BYTE*)pDstBits;
		m_texture.m_ptr = (BYTE*)pDstBits;

		// 将源数据拷贝到新纹理中
		pDstBits += m_texture.m_nStride;
		pDstBits += 4; 
		byte* pSrcBits = pBuffer->m_pScan0;

		for (int y = 0; y < pBuffer->m_nHeight; y++)
		{
			memcpy(pDstBits, pSrcBits, pBuffer->m_nStride);
			pDstBits += m_texture.m_nStride;
			pSrcBits += pBuffer->m_nStride;
		}
	}
	else
	{
...
void TextureMappingBase::SetTextureRect(RECT* prc)
{
	SetTexturePointA(prc->left, prc->top);
	SetTexturePointB(prc->right-1, prc->top);
	SetTexturePointC(prc->right-1, prc->bottom-1);
	SetTexturePointD(prc->left, prc->bottom-1);
}
