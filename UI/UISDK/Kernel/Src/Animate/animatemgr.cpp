...
void  AnimateManager::RemoveStoryboard(IMessage* pMsg, int nId)
{
    if (NULL == pMsg)
        return;
    ...
    ObjectStoryboard* pObjTimeline = *iter;
    RemoveStoryboard(pObjTimeline->FindStoryboard2(nId));
}
...
IStoryboard*  ObjectStoryboard::FindStoryboard2(int nId)
{
    int nIndex = FindStoryboard(nId);
    if (-1 == nIndex)
        return NULL;

    return m_pStoryboardList[nIndex];
}   
