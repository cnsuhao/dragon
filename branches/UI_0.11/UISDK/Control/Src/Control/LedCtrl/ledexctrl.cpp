...
HRESULT  LEDExCtrl::FinalConstruct(IUIApplication* p)
{
    DO_PARENT_PROCESS(ILEDExCtrl, IButtonBase);

    ObjStyle s = {0};
    s.transparent = 1;
    s.default_transparent = 1;
    m_pILEDExCtrl->ModifyObjectStyle(&s, 0);
    return S_OK;
}
...
void  LEDExCtrl::SetAttribute(IMapAttribute* pMapAttr, bool bReload)
{
    ...
    LPCTSTR szText = pMapAttr->GetAttr(XML_LEDCTRL_INDEXMAP, true);
    if (szText)
        strIndexMap = szText;
...
{
    REGION4 rcNonClient;
    m_pILEDExCtrl->GetNonClientRegion(&rcNonClient);
...
// 设置要显示的文本
void LEDExCtrl::SetText(LPCTSTR szText, bool bUpdate)
{
    m_vecPos.clear();
    m_strText.clear();
    if (szText)
    ...
    for (int i = 0; i < nLength; i++)
    {
        ...
        {
            UI_LOG_WARN(_T("无效的字符索引:%c"), m_strText[i]);
        }
    }	
...
void LEDExCtrl::OnPaint(IRenderTarget* pRenderTarget)
{
    ...
    CRect  rcClient;
    m_pILEDExCtrl->GetObjectClientRect(&rcClient);
...
void  LEDExCtrl::SetItemInfo2(LPCTSTR szCharIndex, LPCTSTR szPosIndex)
{
    ...
    {            
        ...
        for (; i < nCount; i++)
        {
            LPCTSTR szPosPair = pEnum->GetText(i);
    ...
    {
        UI_LOG_WARN(_T("Invalid Format, char:%s, pos:%s"), szCharIndex, szPosIndex);
    }
    SAFE_RELEASE(pEnum);
...
// 每一项大小相同时调用
void  LEDExCtrl::SetItemInfo3(LPCTSTR szCharIndex)
{
