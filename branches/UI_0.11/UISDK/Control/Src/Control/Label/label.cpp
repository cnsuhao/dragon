...
HRESULT  Label::FinalConstruct(IUIApplication* p)
{
...
ITextRenderBase*  Label::GetTextRender()
{
    ITextRenderBase* p = m_pILabel->GetTextRender();
    if (!p && m_strText.length() > 0)
        p = m_pILabel->CreateDefaultTextRender();
...
void Label::OnPaint(IRenderTarget* pRenderTarget)
{
    ...
    {
        ITextRenderBase* p = GetTextRender();
        if (p)
...
HRESULT  PictureCtrl::FinalConstruct(IUIApplication* p)
{
...
HRESULT  GifCtrl::FinalConstruct(IUIApplication* p)
{
