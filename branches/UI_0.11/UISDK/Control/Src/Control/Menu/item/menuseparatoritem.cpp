...
void  MenuSeparatorItemShareData::SetAttribute(IMapAttribute* pMapAttrib, bool bReload)
{
    ...
    LPCTSTR szText = pMapAttrib->GetAttr(XML_MENU_SEPARATOR_RENDEER_PREFIX XML_RENDER_TYPE, true);
    if (szText)
    {
        SAFE_RELEASE(m_pSeparatorRender);
        ...
        if (m_pSeparatorRender)
        {
            ...
            data.szPrefix = XML_MENU_SEPARATOR_RENDEER_PREFIX;
            data.nFlags = SERIALIZEFLAG_LOAD|SERIALIZEFLAG_LOAD_ERASEATTR;
            m_pSeparatorRender->Serialize(&data);
        }
        else
        {
            UI_LOG_WARN( _T("Invalid %s=\"%s\""), XML_MENU_SEPARATOR_RENDEER_PREFIX XML_RENDER_TYPE, szText);
        }
    }
