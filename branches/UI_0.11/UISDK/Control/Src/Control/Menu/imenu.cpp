...
namespace UI
{
UI_IMPLEMENT_Ixxx_INTERFACE_ACROSSMODULE(IMenuSeparatorItem, MenuSeparatorItem, IListItemBase);
UI_IMPLEMENT_Ixxx_INTERFACE_ACROSSMODULE(IMenuStringItem,    MenuStringItem,    IListItemBase);
UI_IMPLEMENT_Ixxx_INTERFACE_ACROSSMODULE(IMenuPopupItem,     MenuPopupItem,     IListItemBase);

UI_IMPLEMENT_Ixxx_INTERFACE_ACROSSMODULE(IMenuStringItemShareData, MenuStringItemShareData, IListItemTypeShareData);
UI_IMPLEMENT_Ixxx_INTERFACE_ACROSSMODULE(IMenuSeparatorItemShareData, MenuSeparatorItemShareData, IListItemTypeShareData);
UI_IMPLEMENT_Ixxx_INTERFACE_ACROSSMODULE(IMenuPopupItemShareData, MenuPopupItemShareData, IListItemTypeShareData);

UI_IMPLEMENT_Ixxx_INTERFACE_ACROSSMODULE(IMenu, Menu, IListCtrlBase)

void  IMenu::OnNewChildElement(IUIElement* pUIElement)
{ 
    m_pMenuImpl->OnNewChildElement(pUIElement); 
}
IMenu*  IMenu::GetSubMenuByPos(int nPos) 
{
    return m_pMenuImpl->GetSubMenuByPos(nPos);
}
IMenu*  IMenu::GetSubMenuById(long lId)  
{ 
    return m_pMenuImpl->GetSubMenuById(lId);
}

IMenuStringItem*  IMenu::AppendString(LPCTSTR szText, UINT nId) 
{ 
    return m_pMenuImpl->AppendString(szText, nId);
}
IMenuSeparatorItem*  IMenu::AppendSeparator(UINT nId) 
{
    return m_pMenuImpl->AppendSeparator(nId); 
}
IMenuPopupItem*  IMenu::AppendPopup(LPCTSTR szText, UINT nId, IMenu* pSubMenu)
{ 
    return m_pMenuImpl->AppendPopup(szText, nId, pSubMenu);
}
