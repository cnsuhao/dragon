...
bool  LogItem::Init()
{
	if (0 == ::SysStringLen(m_logitem.logFile) && !(m_logitem.uLogFlag & LOG_FLAG_CONSOLE))
		return false;
...
void LogItem::Log(LOGDATA* pData)
{
	if (false == m_logitem.bEnable)
		return;

	if (pData->level < m_logitem.logLevel)
		return;
	...
	if (m_logitem.uLogFlag & LOG_FLAG_CONSOLE || m_logitem.uLogFlag & LOG_FLAG_DBGVIEW || f.is_open())
	{
		...
		strContent.append(LogItem::LevelToString(pData->level));
		strContent.append(_T(" "));
...
#define LOG_FILEFUNCTIONLINE (LOG_FLAG_FILE|LOG_FLAG_FUNCTION|LOG_FLAG_LINE)

		if (m_logitem.uLogFlag & LOG_FILEFUNCTIONLINE)
		{
			strContent.append(TEXT("\n\t"));
			if (m_logitem.uLogFlag & LOG_FLAG_FILE && pData->szFile)
			{
				strContent.append((TCHAR*)pData->szFile);
				strContent.append(TEXT(" "));
			}
			if (m_logitem.uLogFlag & LOG_FLAG_FUNCTION && pData->szFunction)
			{
				strContent.append((TCHAR*)pData->szFunction);
				strContent.append(TEXT(" "));
			}
			if (m_logitem.uLogFlag & LOG_FLAG_LINE && pData->lLine >= 0)
			{
				TCHAR szBuf[16] = {0};
				strContent.append((TCHAR*)_itot(pData->lLine, szBuf, 10));
			}
			strContent.append(TEXT("\n\t"));
		}
		strContent.append((TCHAR*)pData->szContent);
	}
	if (m_logitem.uLogFlag & LOG_FLAG_CONSOLE)
	{
		...
		_logToStream( wcout, pData->level, strContent);
	}
	
	if (f.is_open())
		_logToStream( f, pData->level, strContent);
