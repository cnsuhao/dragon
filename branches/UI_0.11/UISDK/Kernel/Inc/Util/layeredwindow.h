...
//         ::UpdateLayeredWindow(m_hHostWnd, NULL, &pt, NULL, m_hLayeredMemDC, &ptMemDC, 0, &bf, nFlag );
	...
	void InitLayered(byte nAlpha = 255)
	{
		if (NULL == m_hHostWnd)
			return;
	...
	void Commit2LayeredWindow()
	{
		...
		bf.AlphaFormat = AC_SRC_ALPHA;        
		bf.BlendFlags  = 0;
		bf.SourceConstantAlpha = (byte)m_nSourceConstantAlpha;

		::UpdateLayeredWindow(m_hHostWnd, NULL, &leftTop, &size, m_hLayeredMemDC, &ptMemDC, 0, &bf, nFlag ); 
	}
