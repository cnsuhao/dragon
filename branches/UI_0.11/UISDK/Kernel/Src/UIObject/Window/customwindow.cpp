...
{
	__super::OnInnerInitWindow();

    // 属性加载时，创建了m_pWindowTransparent。在这里进行初始化
...
void  CustomWindow::OnSerialize(SERIALIZEDATA* pData)
{
    ...
    s.AddEnum(XML_WINDOW_TRANSPARENT_TYPE, this,
        ...
        ->AddOption(WINDOW_TRANSPARENT_TYPE_NORMAL,       EMPTYTEXT)
...
void  CustomWindow::SetWindowTransparent(IWindowTransparent* pMode)
{
    ...
    m_oWindowRender.OnWindowTransparentModeChanged(m_pWindowTransparent);
}
