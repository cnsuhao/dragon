...
GifImageRender::GifImageRender(GifImageBase* pGifImage, Gif_Timer_Notify* pNotify, IUIApplication*  pUIApp)
{
...
// 		Gif_Timer_Factory::GetGifTimerEngine(false)->kill_timer((int)this, true);
	GifTimerManager*  pTimerMgr = m_pUIApp->GetImpl()->GetGifTimerMgr ();
...
//		成功返回TRUE，失败返回FALSE
//
GifImageRender* GifImageBase::AddRender(Gif_Timer_Notify* pNotify, IUIApplication*  pUIApp, int* pIndex)
{
	if (NULL == pNotify)
		return NULL;
...
void GifImageRender::commit(HDC hDC, int x, int y)
{
	...
	case Gif_Timer_Notify_Send_Msg:
		{		
			UIMSG msg;
			msg.message = UI_WM_GIFFRAME_TICK;
